...
//        datapool.getDatastore().delete(datapool.applications());
//        datapool.getDatastore().delete(datapool.users());
//        datapool.getDatastore().delete(datapool.companies());
    }
    ...
    public void createCompany() {
        ...
        assertThat(datapool.companies().field("name").equal(companyName).get(), is(not(nullValue())));
    }
    ...
    public void createUser() {
        ...
        assertThat(datapool.users().field("name").equal(userName).get().email, is(email));
    }
    ...
    public void addUserToCompany() {
        ...
        try {
            user.company = datapool.companies().field("name").equal(companyName).get();
    ...
    public void addUsersToCompany1() {
        Company company1 = datapool.companies().field("name").equal("company 1").get();

        for (User user : datapool.users().field("name").startsWith("user ").fetch()) {
            user.company = company1;
            datapool.save(user);
        }

        assertThat(datapool.users().field("company").equal(company1).asList(), is(not(empty())));
    }

    @Test(dependsOnMethods = "setupDatapool")
    public void addAUserAndACompany() {
        User user = new User("A User with a Company", "auserwithacompany@testmail.com");
    ...
    public void createApplication() {
        ...
        assertThat(datapool.applications().field("name").equal(applicationName).get(), is(not(nullValue())));
    }
    ...
    public void addApplicationToUser() {
        ...
        Application application = datapool.applications().field("name").equal(applicationName).get();
        ...
        assertThat(datapool.users().field("applications").hasThisElement(application).asList(), is(not(empty())));
        assertThat(datapool.users().field("applications").hasThisElement(application).get(), is(user));
    }
}
